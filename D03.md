...
* **`document.createElement('li')`** ：新しい `<li>` 要素をその場で生成します。文字列を組み立てなくても、DOM ノードとして扱えるので後からイベントや属性を追加しやすいのがポイントです。
...
### 補足：`createElement` と `innerHTML` のどちらを選ぶ？

`renderTableMemo` では文字列を `innerHTML` にまとめて流し込む方法を採用していますが、先ほどの `<ul>` 版では `document.createElement('li')` と `appendChild(item)` を使いました。それぞれの長所を把握しておくと、本編の `renderEntries` を読み解きやすくなります。

* DOM API (`createElement`, `appendChild`, `textContent` など)
  * 1 行ずつ生成するので最初は手数が多く感じますが、**後から個別のイベントを付けたり、データ属性を持たせたりする拡張がしやすい**のが利点です。
  * `textContent` に代入する限り、ユーザーが入力した `&` や `<script>` が HTML として解釈されないため、**XSS（クロスサイトスクリプティング）を避けやすい**作りになります。
* `innerHTML`
  * 配列からテンプレート文字列を組み立てるだけなので、**`entries.map(...).join('')` のようなフローに慣れていると読みやすい**です。
  * まとめて HTML を差し替えるため、再描画時に既存の要素ごと置き換えたいケース（表全体を書き換えるなど）では素直に書けます。

状況に応じてどちらを選んでも構いませんが、「イベントを後付けしたい（削除ボタンなど）」「ユーザー入力をそのまま埋め込みたくない」といった理由が強ければ DOM API を選び、配列から**純粋に見た目だけを再構築したい**ときは `innerHTML` 版を採用する、という判断基準を持っておくと迷いにくくなります。
...
`createdAt` というフィールドには `Date.now()` の戻り値を入れます。`Date.now()` は「今この瞬間だけの連番」をくれるストップウォッチのような関数で、呼び出すたびに数字が必ず大きくなります。同じタイミングで 2 回押さない限り重ならないので、ほぼユニークな ID の代わりになります。仕組みをもう少し深掘りすると、この数字は **1970 年 1 月 1 日 00:00:00 UTC からの経過ミリ秒** を表しており、時間が進むほど増え続ける性質をそのまま利用しています。そのため `b.createdAt - a.createdAt` のように差を取れば、より新しい記録ほど大きい値を持っていることを利用して簡単に並べ替えられます。
