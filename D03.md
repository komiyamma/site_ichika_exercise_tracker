...
時刻表示をしたい場合は、`new Date(entry.createdAt)` を `toLocaleTimeString` などで整形して `<td>` に追加すれば OK です。`script.js` でも `createdAt` を付けて `filteredEntries.sort(...)` で新しい順にしてからテーブルへ流し込んでいるので、ここで試した改善がそのまま本編へつながります。
