...
### ✨ `map`と`join`でテンプレ風に整える

`for (const entry of entries) {...}` で一行ずつ手書きするので十分ですが、テンプレート的に書きたいときは`entries.map(...).join('')`を使っても問題ありません。`map`で各行のHTML文字列を配列にし、最後に`join('')`でひとつの大きな文字列にまとめる方法です。  
...
しかし、mapとjoinで書けば「速くなる」わけではないので注意しましょう。  
(むしろこのシーンではmap+joinの方がほぼ確実に遅くなるでしょう）  
これは、JavaScriptが`tableHtml += ...` のような文字列連結が起きた時、巧みな最適化がなされており、  
反復したとしても、「速度とメモリの両面」でハンデになりくいためです。
  
よほどデータ件数がおおくない限りは、自分にとって読みやすい記述、自然な思考に寄り添う記述を心がけてください。  
データ件数が多い場合は、「どちらが速いのか」を実際に計測して選択するのが良いでしょう。
