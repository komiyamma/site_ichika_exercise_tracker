...
ちなみに、` for (const entry of entries) {...}` でうｓが、同じ処理をもう少しテンプレート的に書きたいときは`entries.map(...).join('')`を使っても構いません。`map`で各行のHTML文字列を配列にし、最後に`join('')`でひとつの大きな文字列にまとめるだけです。
...
`map`はいったん各`<tr>`の文字列を配列に確保するぶん、件数が何万件もあるようなケースでは余分なメモリを食う可能性はありますが、今回のように日次で数件ずつ積み上げていく使い方ならほとんど差は出ません。好みや読みやすさで書き分けてしまって大丈夫ですよ。
