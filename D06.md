...
このように `closest('button[data-id]')` を使って「押されたボタン（もしくはその子要素）」を簡単に特定し、`data-id` を取り出す仕組みに発展しています。さらに保存キーも `WORKOUT_STORAGE_KEY` という定数で統一されています。まずはこの章のシンプルな実装で動きを理解し、慣れてきたらクラス名の付け方や保存キー、`closest` を使った判定などを少しずつ取り入れてみましょうね。
