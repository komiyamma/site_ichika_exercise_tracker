...
2. **`generateEntryId`** ：エントリーデータに一意のIDを付与する関数です。`Date.now()`で現在の時間（ミリ秒）を返し、`getFormData` 側で文字列化してIDに使いながら、同じ値を `createdAt` としても保持します。
