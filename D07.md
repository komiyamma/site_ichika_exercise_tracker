...
> 💡 メモ：`parseInt(value, 10) || 0` は「数値に直して、うまくいかなければ 0 を選ぶ」という省略記法です。`parseInt` が空欄を読むと結果は `NaN`（Not a Number）になり、これは JavaScript では偽扱い。そのため `|| 0` の部分が 0 を返し、数字欄が空でも安全に 0 が入ります。もしこの書き方がまだ難しく感じたら、`if (value === '') { return 0; }` のように段階を分けて書いても同じ動きになります。
