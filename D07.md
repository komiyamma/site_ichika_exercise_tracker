...
> 💡 メモ：`parseInt(value, 10) || 0` は「数値に直して、うまくいかなければ 0 を選ぶ」という省略記法です。`parseInt` が空欄を読むと結果は `NaN`（Not a Number）になり、これは JavaScript では偽扱い。そのため `|| 0` の部分が 0 を返し、数字欄が空でも安全に 0 が入ります。もしこの書き方がまだ難しく感じたら、以下のように段階を分けたコードを先に動かしてみると、フォールバックの仕組みがイメージしやすくなります。
>
> ```javascript
...
> ```
>
> あるいは三項演算子を使って `const minutes = minutesInputElement.value === '' ? 0 : parseInt(minutesInputElement.value, 10);` と書いても同じ結果になります。これらと見比べることで、「`||` は左側が `NaN`（= falsy）だったときに右側へフォールバックする」という省略形だと捉えられるようになります。
...
> ⚠️ メモ：`localStorage` のデータが壊れていると、ここで `JSON.parse` がエラーを投げて処理が止まります。別ページのスクリプトやブラウザ拡張が内容を書き換えたケースなどが代表例です。この章では「そういう事故もあるんだな」と知っておき、13章で `try...catch` を足してガードする伏線として覚えておきましょう。
