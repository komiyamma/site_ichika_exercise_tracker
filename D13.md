...
> ⚠️ メモ：たとえばプライベートブラウズ中の Safari では `localStorage` への書き込みが禁止されており、そのまま `JSON.parse` しようとすると例外が投げられます。`try...catch` が無いと処理が途中で止まり、画面が更新されなくなります。`catch` の中で空の配列を返してログに残しておけば、「保存はできなかったけれどアプリは動き続ける」という安全策をとれます。
