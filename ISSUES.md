...
### 2. `addEventListener`と`onclick`の混在に関する説明不足

**問題の詳細：**
- D08.mdで`addEventListener`を推奨している
- しかしscript.jsの削除ボタンでは`onclick="removeButtonClick('${id}')"`を使用
- なぜ使い分けているのか説明がない
...
```markdown
### 🤔 なぜ削除ボタンは`onclick`を使うの？

「あれ？D08では`addEventListener`を使ったのに、
削除ボタンは`onclick`を使ってる！」と思いましたよね。

実は、これには理由があります：

**静的な要素（最初からHTMLにある）**
→ `addEventListener`で登録するのが一般的
→ フォーム、フィルターボタンなど

**動的な要素（JavaScriptで後から作る）**
→ `onclick`属性で直接指定する方がシンプル
→ 削除ボタン（renderEntryTableで毎回作り直される）

削除ボタンは`renderEntryTable()`の中で毎回作り直されます。
`addEventListener`を使うと、ボタンが作られるたびに
イベントを登録し直す必要があり、複雑になります。

**より高度な方法：**
「イベント委譲」という技術を使えば、親要素に1つだけ
イベントリスナーを登録して、すべての削除ボタンを
処理できます。興味がある人は調べてみてください！
```
...
2. **`addEventListener`と`onclick`の使い分け説明**
   - D10に理由を明記
   - 動的要素と静的要素の違いを説明
...
1. **教材とコードの一貫性を保つ**（sort/toSorted、addEventListener/onclick）
