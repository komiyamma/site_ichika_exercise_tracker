...
> 🔎 補足：`event.target.matches('.delete-button')` で何をしている？
>
> クリックされた要素（`event.target`）が本当に削除ボタンなのかを CSS セレクターで判定しています。`matches` が `false` なら、例えばセル自体や `<tbody>` がクリックされたケースなので早めに `return` して終了します。逆に `true` のときは `.delete-button` クラスを持つ `<button>` が押されたと分かるので、`dataset.id` を読んで削除処理へ進みます。こうして「代表で受け取ったイベントの中から、必要なものだけ通すフィルター」を挟むのがイベントデリゲーションの基本パターンです。

> � 本編 (`script.js`) との違いは？
>
> 完成版ではテーブルをまるごと `innerHTML` で描画し直した直後に `onclick="removeButtonClick('id')"` を仕込む手法を採用しています。最終コードでは `removeButtonClick` というグローバル関数が既に用意されているため、文字列で ID を渡すだけのシンプルな書き味を優先した形です。第4章の検証コードでは「イベントデリゲーション」という別の選択肢を体験することが目的なので、`tbody` 1 箇所にリスナーを置いて、`event.target.matches('.delete-button')` で本当に削除ボタンが押されたかを判定する練習をしています。どちらの書き方でも削除機能は実現できますが、状況に応じて書き分けられるように両方を紹介しています。
