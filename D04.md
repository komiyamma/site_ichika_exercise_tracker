...
> * ボタンごとに `addEventListener` する手間がなくなり、コードもブラウザの仕事量も少なくて済みます。これが **イベントデリゲーション** と呼ばれるゆえんです。
...
* クリックイベントは `tbody` に 1 回登録しておき、実際に押されたボタンは `event.target` で判定する形がシンプルでした。新しく足したボタンも自動で拾えて、ボタンごとに処理を書き足す必要がありません。これが **イベントデリゲーション** と呼ばれるパターンです。
