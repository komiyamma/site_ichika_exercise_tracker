# 🌟 第4章：`createdAt` を ID にして削除ボタンを検証しよう
...
```javascript
const filterEntries = tableEntries.filter((entry) => entry.id !== targetId);
tableEntries.length = 0; // 既存配列の中身を全てクリアする定番の書き方
tableEntries.push(...filterEntries);
renderTableMemo();
```

これで削除ボタンを押すたびに行が消えるようになりました。配列の参照を保つために `length = 0` → `push(...filterEntries)` としている点は第3章のレンダリングパターンと同じ思想です。
  ...
  <body>
    ...
    <script>
      ...
      function handleDeleteButtonClick(event) {
        ...
        // 既存配列の中身を全てクリアする定番の書き方
        // メソッド的には存在しないが、イメージとしては tableEntries.clear() みたいなものだと思えば良い
      ...
      function renderTableMemo() {
        ...
        for (const entry of sortedEntries) {
          const recordedAt = new Date(entry.createdAt).toLocaleTimeString('ja-JP', {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit',
          });
...
ポイントは、`tableEntries` を直接再代入する代わりに `length = 0` → `push(...filterEntries)` で中身を差し替えている点です。これでグローバル配列の参照を保ったまま内容を更新できます（`tableEntries.splice(0, tableEntries.length, ...filterEntries);` と書いても同じ意味になります）。
...
* クリックイベントは `tbody` に 1 回登録しておき、実際に押されたボタンは `event.target` で判定する形がシンプルでした。これが **イベントデリゲーション** と呼ばれるパターンです。
