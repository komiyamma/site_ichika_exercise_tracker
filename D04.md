...
「もし、ものすごくたくさんの記録を、すっごく速く追加したらIDが被らない？（１ミリ秒以内に）」と気になった方は、とても良い視点です！

プロの現場では、そういった非常にまれなケースも想定して、「極めて重複しにくいID生成(UUID等)」、あるいは「絶対に重複しないID生成(UUID等+重複チェック)」にします。

ただ、今回の練習では、シンプルで分かりやすい `Date.now()` をそのままIDとして使っていきましょう！
