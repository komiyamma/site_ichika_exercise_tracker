...
function renderEntryTable() {
    ...
    // --- 心の旅 ---
    // 「`entries = entries.filter(...)`みたいに、元の配列を直接書き換えちゃえば、変数が一つ減ってシンプルになるんじゃないかな？って思った。
    //   でも、そうすると、この関数の後半で`sort`するときに、元の全データが失われちゃう。
    //   `filteredEntries`っていう別の変数に絞り込んだ結果を入れておくことで、『元の全データ』と『表示用の絞り込んだデータ』を両方とも保持できる。
    //   だから、並び替えたり、件数を数えたりするときに、混乱しなくて済むんだな。変数をむやみに上書きしない方が、データの流れが追いやすくて安全だ。」
...
### 💬 「他の“日”には内緒よ？<br>　 　 今日はこの日だけの特別上映会🎥<br>　 　 思い出は、こっそり選んだ日の中で育つの📅」
